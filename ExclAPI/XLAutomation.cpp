...
BOOL CXLAutomation::StartExcel()
{
	...
	{
		MessageBox(NULL, _T("Cannot start an instance of Excel for Automation."), L"Failed", MB_OK | MB_ICONSTOP);
		return FALSE;
	}
...
void CXLAutomation::ReleaseVariant(VARIANTARG *pvarg)
{
	...
	if (V_ISARRAY(pvarg)) 
	{
		...
		{
			MessageBox(NULL, _T("ReleaseVariant: Array contains non-variant type"), L"Failed", MB_OK | MB_ICONSTOP);
		}
	...
	else 
	{
		switch (vt) 
		{
			...
			default:
				MessageBox(NULL, _T("ReleaseVariant: Unknown type"), L"Failed", MB_OK | MB_ICONSTOP);
				break;
		}
	}
